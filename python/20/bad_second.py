...
# I did not understand you can activate a cheat and not immediately walk into a wall.
# I like this code though so it'll stay
...
def reachable_walls(
    starting: tuple[int, int], walls: set[tuple[int, int]], limit=18
) -> dict[tuple[int, int], int]:
    distances: dict[tuple[int, int], int] = {}
    exploring = [(0, starting)]
    while len(exploring) > 0:
        ...
        for direction in dirs:
            ...
            #     continue
            heappush(exploring, (steps + 1, candidate))
    wall_distances: dict[tuple[int, int], int] = {}
    for pos, dst in distances.items():
        if pos in walls:
            wall_distances[pos] = dst
    return wall_distances


def display(walls, wall_hacks, start, end, notable=None) -> None:
    width = 0
    height = 0
    for x, y in walls:
        width = max(width, x + 1)
...
def solve(data: Data, limit: int) -> int:
    ...
    reachables: dict[tuple[int, int], dict[tuple[int, int], int]] = {}
    ...
    for pos, dfs in distance_from_start.items():
        ...
        for one_direction in dirs:
            ...
            if one_step not in data.walls:
                ...
                continue
            if one_step not in reachables:
                reachables[one_step] = reachable_walls(one_step, data.walls)
            for zoop_step, zoop_dist in reachables[one_step].items():
                for out_direction in dirs:
                    out_step = step(zoop_step, out_direction)
                    if out_step == pos:
                        continue
                    if out_step not in distance_from_end:
                        # we are looking for a cheat not clipping outside the arena
                        continue
                    dfe = distance_from_end[out_step]
                    distance = dfs + dfe + 2 + zoop_dist
                    if distance <= cutoff:
                        wall_hacks.add(one_step)
                        wall_hacks.add(zoop_step)
                        cheat = (pos, out_step)
                        if cheat not in cheats:
                            print(f"Saving {no_cheat-distance} @ {pos} - {out_step}")
